        ...
        public partial class EidssSecurityManager
        {
            ...
            private Dictionary<string, bool> ParsePermissions(DataTable dt)
            {
                ...
                foreach (DataRow row in dt.Rows)
                {
                    ...
                    string sf = ((EIDSSPermissionObject)(Convert.ToInt32(row["idfsSystemFunction"]))).ToString();
            ...
            private void setEidssUser(DataTable dt)
            {
                ...
                EidssUserContext.User.Organization = Utils.Str(dt.Rows[0]["strLoginOrganization"]);
            ...
            public int LogIn(string ticketId)
            {
                using (DbManagerProxy manager = DbManagerFactory.Factory.Create(ModelUserContext.Instance))
                {
                    try
            ...
            public int LogIn(string organization, string userName, string password)
            {
                ...
                using (DbManagerProxy manager = DbManagerFactory.Factory.Create(ModelUserContext.Instance))
                {
                    try
                    {
                        ...
                        if (res == 0)
                        {
                            if (BaseSettings.UseOrganizationInLogin && Utils.Str(organization).ToLower() != Utils.Str(dt.Rows[0]["strLoginOrganization"]).ToLower())
                            {
                                return 2;
                            }
                            PerformLogin(dt);
