...
namespace eidss.model.AVR.Db
{
    public static class QueryHelper
    {
        ...
        public static DataTable GetInnerQueryResult(DbManagerProxy manager, string queryString)
        {
            return GetInnerQueryResult(manager, queryString, ModelUserContext.CurrentLanguage, c => c.ExecuteDataTable());
        }
        ...
        public static void DropAndCreateArchiveQuery(DbManagerProxy manager, DbManagerProxy archiveManager, long queryId)
        {
            ...
            string queryFunctionName = GetQueryFunctionName(manager, queryId);
        ...
        public static DataTable GetTableWithoutCopiedColumns(DataTable sourceTable)
        {
            IEnumerable<DataColumn> columns = sourceTable.Columns.Cast<DataColumn>();
            string[] columnNames =
                columns
                    .Select(c => c.ColumnName)
                    .Where(n => !n.EndsWith(CopySuffix))
                    .ToArray();

            DataTable destTable = (new DataView(sourceTable)).ToTable(sourceTable.TableName, false, columnNames);
