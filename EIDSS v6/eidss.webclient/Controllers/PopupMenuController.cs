...
namespace eidss.webclient.Controllers
{
    ...
    public class PopupMenuController : BvController
    {
        ...
        {
            var objs = idents.Split(',').Select(c => ModelStorage.Get(Session.SessionID, long.Parse(c), "_" + root) as IObject).ToList();
        ...
        private List<MenuItem> MenuList(List<IObject> objs, long root, string idents)
        {
            var o = objs[0];
            ...
            var list =
                meta.Actions.Where(c => c.PanelType == ActionsPanelType.ContextMenu && (c.AppType == ActionsAppType.Web || c.AppType == ActionsAppType.All))
        ...
        {
            var ret = actions.FirstOrDefault(c => c.PanelType == ActionsPanelType.ContextMenu && c.Name == name);
            if (ret == null)
            {
        ...
        public ActionResult Select(long root, string idents, string name)
        {
            ...
            using (DbManagerProxy manager = DbManagerFactory.Factory.Create(EidssUserContext.Instance))
            {
                foreach (var s in ids)
                {
                    ...
                    var o = ModelStorage.Get(Session.SessionID, id, "_" + root) as IObject;
