...
public class HumanCase implements Parcelable, IValidatable {
    private long _id;
	...
	public ValidateCode Validate() {
        ...
        if (getFamilyName() == null || getFamilyName().trim().length() == 0)
        	return ValidateCode.LastNameMandatory;
        if (getRegionCurrentResidence() == 0)
        	return ValidateCode.RegionMandatory;
        else if (getRayonCurrentResidence() == 0)
        	return ValidateCode.RayonMandatory;
        else if (getDateofBirth() != null && getDateofBirth().after(DateHelpers.Today()))
        	return ValidateCode.DateOfBirthCheckCurrent;
        else if (getOnSetDate() != null && getOnSetDate().after(DateHelpers.Today()))
        	return ValidateCode.DateOfSymptomCheckCurrent;
        else if (getTentativeDiagnosisDate() != null && getTentativeDiagnosisDate().after(DateHelpers.Today()))
        	return ValidateCode.DateOfDiagnosisCheckCurrent;
        else if (getTentativeDiagnosisDate() != null && getNotificationDate() != null && getTentativeDiagnosisDate().after(getNotificationDate()))
        	return ValidateCode.DateOfDiagnosisCheckNotification;
        else if (getDateofBirth() != null && getTentativeDiagnosisDate() != null && getDateofBirth().after(getTentativeDiagnosisDate()))
        	return ValidateCode.DateOfBirthCheckDiagnosis;
        else if (getDateofBirth() != null && getNotificationDate() != null && getDateofBirth().after(getNotificationDate()))
        	return ValidateCode.DateOfBirthCheckNotification;
        else if (getOnSetDate() != null && getTentativeDiagnosisDate() != null && getOnSetDate().after(getTentativeDiagnosisDate()))
        	return ValidateCode.DateOfSymptomCheckDiagnosis;
        else if ((getHumanAgeType() == 0 && getPatientAge() != 0) || (getHumanAgeType() != 0 && getPatientAge() == 0))
        	return ValidateCode.AgeType;
	...
	public void writeXml(long country, XmlSerializer serializer) throws IllegalArgumentException, IllegalStateException, IOException{
        ...
        if (getFirstName() != null) 
	        serializer.attribute("", "firstName", getFirstName());
        if (getFamilyName() != null) 
	        serializer.attribute("", "lastName", getFamilyName());
        if (getDateofBirth() != null) 
	        serializer.attribute("", "dateOfBirth", formatterDateTime.format(getDateofBirth()).replace(' ', 'T'));
        if (getPatientAge() != 0) 
	        serializer.attribute("", "patientAge", String.valueOf(getPatientAge()));
        if (getHumanAgeType() != 0) 
	        serializer.attribute("", "patientAgeType", String.valueOf(getHumanAgeType()));
        if (getHumanGender() != 0) 
	        serializer.attribute("", "patientGender", String.valueOf(getHumanGender()));
        
        serializer.attribute("", "country", String.valueOf(country));
        if (getRegionCurrentResidence() != 0) 
	        serializer.attribute("", "region", String.valueOf(getRegionCurrentResidence()));
        if (getRayonCurrentResidence() != 0) 
	        serializer.attribute("", "rayon", String.valueOf(getRayonCurrentResidence()));
        if (getSettlementCurrentResidence() != 0) 
	        serializer.attribute("", "settlement", String.valueOf(getSettlementCurrentResidence()));
        if (getBuilding() != null) 
        ...
        if (getHomePhone() != null) 
			serializer.attribute("", "homePhone", getHomePhone());
        if (getOnSetDate() != null) 
	        serializer.attribute("", "onSetDate", formatterDateTime.format(getOnSetDate()).replace(' ', 'T'));
        if (getFinalState() != 0) 
	        serializer.attribute("", "patientState", String.valueOf(getFinalState()));
        if (getHospitalizationStatus() != 0) 
	        serializer.attribute("", "hospitalization", String.valueOf(getHospitalizationStatus()));
        serializer.endTag("", "case");
	}
	
	public static void writeXml(List<HumanCase> hcs, long country, XmlSerializer serializer) throws IllegalArgumentException, IllegalStateException, IOException{
        ...
        for (HumanCase hc: hcs){
        	if (hc.getStatus() == CaseStatus.NEW || hc.getStatus() == CaseStatus.CHANGED || hc.getStatus() == CaseStatus.UNLOADED){
        		hc.writeXml(country, serializer);
        	}
        }
