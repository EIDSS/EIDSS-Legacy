    ...
    public partial class Template
    {
        public void RulesCheckRoutines(List<Rule> rules, FFPresenterModel ff, out List<Rule> executedRules, out List<Rule> enablingRules)
        {
            ...
            foreach (var rule in rules)
            {
                ...
                var value1 = ff.ActivityParameters.GetActivityParameter(false, idObservation, rule.Parameters[0].idfsParameter);
                if ((rule.idfsRuleFunction != (long)FFRuleFunctions.Empty) && (Utils.IsEmpty(value1) || Utils.IsEmpty(value1.varValue))) return;
                ActivityParameter value2 = null;
                if (rule.intNumberOfParameters == 2)
                {
                    value2 = ff.ActivityParameters.GetActivityParameter(false, idObservation, rule.Parameters[1].idfsParameter);
                ...
                switch (rule.idfsRuleFunction)
                {
                    ...
                    case (long)FFRuleFunctions.Value:
                        ...
                        foreach (var constant in rule.Constants)
                        {
                            //сравним как строки
                            if (!constant.varConstant.ToString().Equals(value1.varValue.ToString())) continue;
                            conditionsDone = true;
                            break;
                        }
                        break;
                ...
                else if (rule.Actions.Any(c => c.idfsRuleAction == (long)FFRuleActions.Disabled))
                {
            ...
            )
        {
            ...
            var rules = Rules.Where(c => c.idfsCheckPoint.HasValue && (c.idfsCheckPoint.Value == (long)checkPointType)).ToList();
            //отбираем те правила, где фигурирует нужный параметр
            //правила без действий не могут быть выполнены
            for (var i = rules.Count - 1; i >= 0; i--)
            {
                var rule = rules[i];
                if (rule.Parameters.All(c => c.idfsParameter != idParameter))
                {
                    rules.Remove(rule);
                }
                else if (rule.Actions.Count == 0)
                {
                    rules.Remove(rule);
                }
