    ...
    public partial class AsCampaign
    {
        public bool ValidateSessionAssignment(AsSession session)
        {
            if (session.idfCampaign.HasValue)
            {
                ...
                {
                    throw new ValidationModelException("msgASSessionAlreadyBelongsToCampaign", "idfCampaign", "idfCampaign", new string[] { "Error" }, typeof(PredicateValidator), false, session);
                }
            }

            if ((session.datStartDate.HasValue && datCampaignDateStart.HasValue && session.datStartDate < datCampaignDateStart) ||
                (session.datEndDate.HasValue && datCampaignDateEnd.HasValue && session.datEndDate > datCampaignDateEnd))
            {
                throw new ValidationModelException("msgCampaignSessionDatesConflict", "idfCampaign", "idfCampaign", new object[] { session.datStartDate, session.datEndDate, datCampaignDateStart, datCampaignDateEnd, "Error" }, typeof(PredicateValidator), false, session);
            }
            ...
            {
                throw new ValidationModelException("errNotMatchSessionDiagnosis", "idfCampaign", "idfCampaign", new string[] { "Confirmation" }, typeof(PredicateValidator), false, session);
            }           
        ...
        private static bool NewDiseaseValidation(AsCampaign campaign, AsDisease item)
        {
            ...
            if (campaign.Diseases.Count(x => x.idfCampaignToDiagnosis != item.idfCampaignToDiagnosis && x.idfsDiagnosis == item.idfsDiagnosis && x.idfsSpeciesType == item.idfsSpeciesType && !x.IsMarkedToDelete) > 0)
                throw new ValidationModelException("AsSession.Diseases.Duplicate_msgId", "idfsSpeciesType", "idfsSpeciesType", null, typeof(PredicateValidator), false, item);
