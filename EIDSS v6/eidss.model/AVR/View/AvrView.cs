...
namespace eidss.model.Avr.View
{
    ...
    public class AvrView : BaseBand
    {
        ...
        // .ctor for Pivot 
        public AvrView(long layoutID, string layoutName)
        {
            m_ViewID = layoutID;
        ...
        {
            get { return m_IsChanged | m_IsNew | m_ToDelete; }
        }
        ...
        public static IEnumerable<AvrViewColumn> GetMapDefChartList(AvrView view)
        {
            return view.GetMapDefChartList();
        }
        ...
        // used only from web
        public void SetTempOrders(string source, string destination, bool isDropBefore)
        {
            ...
            if (colSource != null)
            {
                ...
                foreach (var col in colSource.Owner.Columns.Where(c => c.UniquePath != source).OrderBy(c => c.Order_Temp.HasValue ? c.Order_Temp : c.Order_ForUse))
                {
            ...
            else
            {
                ...
                {
                    int i = 1;
                    foreach (var col in bandSource.Owner.Bands.Where(c => c.UniquePath != source).OrderBy(c => c.Order_Temp.HasValue ? c.Order_Temp : c.Order_ForUse))
                    {
                        if (isDropBefore && col.UniquePath == destination)
