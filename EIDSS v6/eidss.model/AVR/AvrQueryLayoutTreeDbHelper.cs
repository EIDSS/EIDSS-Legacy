...
namespace eidss.model.Avr
{
    ...
    {
        private static readonly string m_DefaultFolderExists;
        private static readonly string m_NationalFolderExists;
        private static readonly string m_DefaultLayoutExists;
        private static readonly string m_NationalLayoutExists;
        ...
        static AvrQueryLayoutTreeDbHelper()
        {
            ...
            m_NationalFolderExists = EidssMessages.Get("msgNoUniqueNatLayoutName",
        ...
        {
            LookupManager.AddObject("Query", null, AvrQueryLookup.Accessor.Instance(null).GetType(), "SelectLookupList");
            using (DbManagerProxy manager = DbManagerFactory.Factory.Create())
            {
                ...
                List<AvrTreeElement> treeElements = lookup
                    .Where(q => !readOnly || q.blnReadOnly)
        ...
        {
            LookupManager.AddObject("LayoutFolder", null, AvrFolderLookup.Accessor.Instance(null).GetType(), "SelectLookupList");
            using (DbManagerProxy manager = DbManagerFactory.Factory.Create())
            {
                ...
                List<AvrTreeElement> treeElements = lookup
                    .Where(f => !readOnly || f.blnReadOnly)
        ...
        {
            LookupManager.AddObject("Layout", null, AvrLayoutLookup.Accessor.Instance(null).GetType(), "SelectLookupList");
            using (DbManagerProxy manager = DbManagerFactory.Factory.Create())
            {
                ...
                List<AvrTreeElement> treeElements = lookup
                    .Where(
                        lay =>
                            (!readOnly || lay.blnReadOnly) &&
                            ((lay.idfPerson.HasValue && lay.idfPerson.Value == employeeId)) || lay.blnShareLayout)
                    .Select(lay => (AvrTreeElement) lay)
                    .ToList();
        ...
        public static void DeleteFolder(long folderId)
        {
            using (DbManagerProxy manager = DbManagerFactory.Factory.Create())
            {
                DbManager command = manager.SetSpCommand("spAsFolderDelete", manager.Parameter("idflFolder", folderId));
                command.ExecuteNonQuery();
            }
            //LookupCache.NotifyChange("LayoutFolder");
            LookupManager.ClearByTable("LayoutFolder");
