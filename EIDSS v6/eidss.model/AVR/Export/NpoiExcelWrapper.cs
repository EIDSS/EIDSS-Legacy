...
namespace eidss.model.Avr.Export
{
    public class NpoiExcelWrapper : IDisposable
    {
        ...
        public static void QueryLineListToExcel(string fileName, DataTable data, ExportType type)
        {
        ...
        public static List<byte[]> QueryLineListToExcel(DataTable data, ExportType type)
        {
            using (var npoiExcelWrapper = new NpoiExcelWrapper(type))
            {
        ...
        public List<byte[]> Export(DataTable data)
        {
            Dictionary<string, byte[]> exportDictionary = ExportToByteArrays(string.Empty, data);
            return exportDictionary.Values.ToList();
        }

        private Dictionary<string, byte[]> ExportToByteArrays(string fileName, DataTable data)
        {
            using (new CultureInfoTransaction(new CultureInfo("en-US")))
            {
                ...
                for (int rowIndex = 0; rowIndex < data.Rows.Count; rowIndex++)
                {
                    ...
                    for (int colIndex = 0; colIndex < data.Columns.Count; colIndex++)
                    {
                        ...
                        if (data.Rows[rowIndex][colIndex] == DBNull.Value)
                        {
                            SetEmptyCellFormat(cell, data.Columns[colIndex].DataType);
                            continue;
                        }
                        SetCellValue(cell, data.Rows[rowIndex][colIndex]);
                    }
                }
                if (fileNameSuffix != "")
                {
        ...
        private static string GetSheetName(DataTable data)
        {
        ...
        private ISheet CreateNewSheet(string sheetName, int sheetIndex, DataTable data)
        {
            if (sheetIndex > 0)
            {
            ...
            for (int colIndex = 0; colIndex < data.Columns.Count; colIndex++)
            {
                DataColumn col = data.Columns[colIndex];
                ICell cell = excelRow.CreateCell(colIndex);
