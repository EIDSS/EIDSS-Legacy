        ...
        BaseConverter<eidss.openapi.contract.VetCase, eidss.model.Schema.VetCase>
    {
        ...
        internal static void Register()
        {
            ...
            Mapper.CreateMap<eidss.model.Schema.VetCase, eidss.openapi.contract.VetCase>()
                ...
                {
                    return m.Farm.FarmTree.Where(i => i.idfParentParty == (long)PartyTypeEnum.Herd && i.IsMarkedToDelete == false).ToList();
                }))
                .ForMember(c => c.Species, e => e.ResolveUsing(m =>
                {
                    return m.Farm.FarmTree.Where(i => i.idfParentParty == (long)PartyTypeEnum.Species && i.IsMarkedToDelete == false).ToList();
                }))
                ;
            ;
        }
