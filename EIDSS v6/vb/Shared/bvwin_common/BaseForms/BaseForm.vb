...
#Region "Public Methods"
    ...
    Public Sub DoClose()
        Try
            ...
            If Not Disposed Then Me.Dispose()
        Catch ex As Exception
...
#Region "Overridable Methods"
    ...
    Private Function IsMandatoryFieldEmpty(ByVal c As Control) As Boolean
        If c.DataBindings.Count > 0 Then
                ...
                Try
                    If _
                        ...
                        If _
                            ...
                            Not bv.common.Core.Utils.IsEmpty(TagHelper.GetTagHelper(c).MandatoryFieldName) Then
                            Return _
                                bv.common.Core.Utils.IsEmpty(
            ...
            End If
        End If
        Return bv.common.Core.Utils.IsEmpty(CType(c, BaseEdit).EditValue) OrElse bv.common.Core.Utils.IsEmpty(c.Text)
    End Function
    ...
    Protected Sub ShowErrorAtValidateMandatoryFields(ByVal c As Control, ByVal caption As String)
        ErrorForm.ShowWarningFormat("ErrMandatoryFieldRequired",
                                    "The field '{0}' is mandatory. You must enter data to this field before form saving",
                                    caption)
        FocusOnField(c)
    End Sub
...
#End Region
    ...
    Protected Overridable Function SavePromptDialog(Optional ByVal DefaultResult As DialogResult = DialogResult.Yes) _
        As DialogResult
        If BaseSettings.ShowSaveDataPrompt AndAlso Not bv.common.Core.Utils.IsCalledFromUnitTest Then _
            'AndAlso HasChanges() 
            Return _
                MessageForm.Show(BvMessages.Get("Save data?"), BvMessages.Get("Confirmation"),
    ...
    Public Overridable Function LoadData(ByRef id As Object) As Boolean Implements IRelatedObject.LoadData
        ...
        Try
            ...
            Try
                ...
                If (Result = False And Not bv.common.Configuration.BaseSettings.ScanFormsMode) Then
        ...
        Finally
            ...
            If Not Me.PermissionObject Is Nothing Then
                If _
                    ((common.Core.Utils.Str(id) <> "SearchMode") AndAlso
                     ((DbService Is Nothing) OrElse
                      (common.Core.Utils.Str(DbService.ID) <>
...
#End Region
    ...
    Protected Function GetLookupValue(ByVal keyFieldValue As Object, ByVal LookupTableName As String,
        ...
        If Not bv.common.Core.Utils.IsEmpty(row) Then
            Return bv.common.Core.Utils.Str(row(DisplayFieldName))
        End If
        Return ""
    End Function
    ...
    Public Overridable Function Delete(ByVal key As Object) As Boolean
        Try
            If Not AuditObject Is Nothing Then
                AuditObject.Key = key
                Dbg.DbgAssert(Not bv.common.Core.Utils.IsEmpty(AuditObject.Key),
            ...
            If DbService.Delete(key) Then
                If Not LookupTableNames Is Nothing Then
                    ...
                    Next
                ElseIf Not AuditObject Is Nothing AndAlso Not bv.common.Core.Utils.IsEmpty(AuditObject.AuditTable) Then
    ...
    Protected Sub StartReplicationAsync(ByVal sender As Object, ByVal args As PostEventArgs)
        AuditManager.StartReplicationAsync(Me.AuditObject)
    End Sub
        ...
        Try
            ...
            If Not events Is Nothing AndAlso events.Count > 0 Then
                ...
                For Each evt As EventInfo In events
                    ...
                    End If
                Next
                Dim startReplication As Boolean = Not bv.common.Core.Utils.IsCalledFromUnitTest AndAlso
                ...
                For Each evt As EventInfo In events
                    If bv.common.Core.Utils.IsEmpty(evt.ID) Then
                        evt.ID = GetKey()
                    End If
                    If startReplication AndAlso evt.Processed = 0 Then
                        AuditObject.EventLog = EventLog 'Transaction will be started asynchronously by StartReplicationAsync after transaction finising
