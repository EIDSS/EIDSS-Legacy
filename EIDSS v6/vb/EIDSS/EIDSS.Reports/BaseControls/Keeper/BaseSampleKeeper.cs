...
namespace EIDSS.Reports.BaseControls.Keeper
{
    public partial class BaseSampleKeeper : BaseReportKeeper
    {
        ...
        protected override BaseReport GenerateReport(DbManagerProxy manager)
        {
            ...
            switch (m_ModelType)
            {
                case SampleReportModelType.HumanLabSampleModel:
                    ...
                    report.SetParameters(manager, humLabModel);
                    break;
                case SampleReportModelType.VetLabSampleModel:
                    ...
                    report.SetParameters(manager, sampleModel);
                    break;
                case SampleReportModelType.LabCaseModel:
                    var caseModel = new LabCaseModel(CurrentCulture.ShortName, tbSampleId.Text, UseArchive);

                    report.SetParameters(manager, caseModel);
                    break;
            }
            return report;
        }
        ...
        protected internal override void ApplyResources(DbManagerProxy manager)
        {
            ...
            if (m_ModelType == SampleReportModelType.LabCaseModel)
            {
                lblSampleID.Text = lblCaseID.Text;
            }
        }
    }
}