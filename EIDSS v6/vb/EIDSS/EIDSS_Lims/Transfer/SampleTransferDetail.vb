...
Public Class SampleTransferDetail
    ...
    Friend WithEvents btnBarcodes As bv.common.win.PopUpButton
    ...
    Private Sub InitializeComponent()
        ...
        Me.btnBarcodes = New bv.common.win.PopUpButton()
        ...
        Me.btnBarcodes.ButtonType = bv.common.win.PopUpButton.PopUpButtonStyles.PrintBarcodes
    ...
    Protected Overrides Sub DefineBinding()
        ...
        Core.LookupBinder.BindOrganizationLookup(Me.cbTransferFrom, Me.baseDataSet, Nothing, HACode.None, False)
        ...
        Core.LookupBinder.BindOrganizationLookup(Me.cbTransferTo, Me.baseDataSet, Nothing, HACode.None)
        ...
        If baseDataSet.Tables("Transfer").Rows(0)("idfsTransferStatus").ToString <> CType(TestStatus.NotStarted, Long).ToString() Then Me.Transfered = True
    ...
    Private Sub ReflectStatus()
        ...
        If Me.Transfered Then
            ...
            End If
            Me.btnTransferIn.Enabled = canIn AndAlso (Me.baseDataSet.Tables("Transfer").Rows(0)("idfSendToOffice").ToString = EidssSiteContext.Instance.OrganizationID.ToString)
            ...
            Me.miBarcodesSampleTransferID.Visible = True
        Else
            ...
            Me.miBarcodesSampleTransferID.Visible = False
        End If
    End Sub
    ...
    Private Sub btnTransferOut_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnTransferOut.Click
        ...
        Try
            ...
            Using trans
                valid = DbService.PostDetail(Me.baseDataSet.Copy, PostType.PerformAdditionalPosting, trans)
                trans.Rollback()
            End Using
        ...
        If valid Then
            ...
            Me.baseDataSet.Tables("Transfer").Rows(0)("idfsTransferStatus") = TestStatus.InProcess
            Me.Transfered = True
    ...
    Private Sub btnTransferIn_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnTransferIn.Click
        ...
        For I = 0 To Me.SamplesGridView.SelectedRowsCount - 1

            row = Me.SamplesGridView.GetDataRow(Me.SamplesGridView.GetSelectedRows(I))
            If IsAccessioned(row) Then Exit Sub
            row("strBarcodeNew") = NumberingService.GetNextNumber(NumberingObject.Sample, Me.DbService.Connection, Nothing)
    ...
    Private Sub SamplesGridView_ShowingEditor(ByVal sender As System.Object, ByVal e As System.ComponentModel.CancelEventArgs) Handles SamplesGridView.ShowingEditor
        ...
        If IsAccessioned(row) Then
            If col Is Me.colLocation OrElse col Is Me.colComment OrElse col Is Me.colConditionReceived OrElse col Is Me.colLabNewID OrElse col Is Me.colReceivedDate OrElse col Is Me.colLaboratory Then
                e.Cancel = False
            End If
        End If
    End Sub
    ...
    Private Sub miBarcodesSampleID_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles miBarcodesSampleID.Click
    ...
    Private Sub miBarcodesSampleTransferID_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles miBarcodesSampleTransferID.Click
        ' Barcode printing  for sample transfer
        If Post(PostType.FinalPosting) Then
            Dim row As DataRow = Me.baseDataSet.Tables("Transfer").Rows(0)
            If row Is Nothing Then Exit Sub
            Dim barcode As Object = row("idfTransferOut")
            If Utils.IsEmpty(barcode) Then Exit Sub

            Dim typeId As Long = CType(NumberingObject.SampleTransfer, Long)
            Dim objectId As Long = CType(barcode, Long)
    ...
    Public Function ValidateSamplesData(ByVal ShowError As Boolean) As Boolean
        If Me.Transfered Then
            ...
            For I = 0 To Me.SamplesGridView.SelectedRowsCount - 1
                ...
                If Not IsCurrentSpecimenRowValid(SamplesGridView.GetSelectedRows(I), ShowError) Then
                    If ShowError Then SamplesGrid.Select()
                    Return False
                End If
    ...
    Public Function IsCurrentSpecimenRowValid(Optional ByVal index As Integer = -1, Optional ByVal showError As Boolean = True) As Boolean
        ...
        If CLng(AccessionCondition.AcceptedPoor).Equals(condition) OrElse CLng(AccessionCondition.Rejected).Equals(condition) Then
            If Utils.Str(row("strCondition")).Trim().Length = 0 Then
                Me.SamplesGridView.FocusedColumn = Me.colComment
                If showError Then
