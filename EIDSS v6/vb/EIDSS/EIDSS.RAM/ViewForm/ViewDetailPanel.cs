...
namespace eidss.avr.ViewForm
{
    public sealed partial class ViewDetailPanel : BaseAvrDetailPresenterPanel, IViewDetailView
    {
        ...
        private bool OpenedFromNewLayout
        {
            get
            {
                return DbService != null && DbService.ParentService != null && DbService.ParentService.IsNewObject;
            }
        }
        ...
        private void BindAggregateFunctions()
        {
            using (SharedPresenter.ContextKeeper.CreateNewContext(ContextValue.BindAggregateFunctions))
            {
                m_ViewDetailPresenter.SharedPresenter.BindAggregateFunctionsInternal(cbAggregate, false, true);
            ...
            try
            {
                ...
                if (col != null)
                {
                    if (cbAggregate.EditValue is long)
                    {
                        ...
                        if (col.DisplayText == EidssMessages.Get("AggregateColumn", "Aggregate Column") || col.DisplayText == m_OldAggrFunction)
                        {
        ...
        private void OpenChartButton_Click(object sender, EventArgs e)
        {
            ...
            using (var chartForm = new ChartDetailForm())
            {
                ...
                var isOkResult = BaseFormManager.ShowModal(chartForm, this, ref id, null, null);
        ...
        private void OpenMapButton_Click(object sender, EventArgs e)
        {
            ...
            if (AvrViewHelper.TryGetCheckedComboSelValue(cbMapDefAdminUnit.EditValue, out columnName))
            {
                ...
                if (TryToPrepareMapData(columnName, out dataSet))
                {
                    ...
                    using (var mapForm = new MapDetailForm(m_ViewDetailPresenter.FillMapAdminUnit, TryToPrepareMapData,
        ...
        {
            // remember columns for future DataTable
            List<AvrViewColumn> mapColumns = m_ViewDetailPresenter.FillSeriesColumns((GridView) grid.MainView, cbMapDefDataChart, cbMapDefDataGradient);
            ...
            if (xAxisRows != null && xAxisRows.Count > 0 && xAxisRows[xAxisRows.Keys.FirstOrDefault()].Table.Columns.Contains(columnName + "hasc"))
            ...
            if (((GridView)grid.MainView).GetSelectedCells().Length <= 1 &&
                 ((cbMapDefDataChart.EditValue.ToString().Length > 0) || (cbMapDefDataGradient != null && cbMapDefDataGradient.EditValue != null && cbMapDefDataGradient.EditValue.ToString().Length > 0)))
                useDefaults = true;
            data = m_ViewDetailPresenter.CreateDataForMap(columnName, mapColumns, xAxisRows, m_AggrCache, (GridView) grid.MainView, useDefaults);
            return true;
        }
        ...
        {
            if (e.FocusedColumn == null || e.FocusedColumn.Tag == null)
            {
                return;
            }

            var aColumn = (AvrViewColumn) e.FocusedColumn.Tag;
            if (aColumn.IsAggregate)
            {
            ...
            if (e.MenuType == GridMenuType.Column)
            {
                ...
                if (e.Menu is GridViewBandMenu)
                {
                    ...
                    if (hi != null && hi.Band.Visible)
                    {
                        ...
                        menu.Items.Insert(pol,
                            CreateMenuItem(EidssMessages.Get("UnfreezeAllBands"), hi.Band, false, m_ViewDetailPresenter.HasFreezedBands(), OnUnFixedAllClick));
                        ...
                        {
                            menu.Items.Add(CreateMenuItem(EidssMessages.Get("btAddAggregateColumn"), hi.Band, true, true, OnAddAggregateColumn));
                        }
                    }
                }
                else if (e.Menu is GridViewColumnMenu)
                {
                    ...
                    if (menu.Column != null && menu.Column.Visible)
                    {
                        ...
                        menu.Items.Add(CreateMenuItem(EidssMessages.Get("btAddAggregateColumn"), menu.Column, true, true, OnAddAggregateColumn));
                        if (((AvrViewColumn)menu.Column.Tag).IsAggregate)
                            menu.Items.Add(CreateMenuItem(EidssMessages.Get("DeleteAggregateColumn"), menu.Column, false, true, OnDeleteAggregateColumn));
                    }
                }
            }
        }
        ...
        private void OnRenameBandItemClick(object sender, EventArgs e)
        {
            ...
            var formLocation = new Point(clBounds.Left, clBounds.Top);
            formLocation = gView.GridControl.PointToScreen(formLocation);
        ...
        private void AddAggregateColumn(BaseBand aBand, CollectionBase columns)
        {
            ...
            var newCol = aBand.AddAggregateColumn();
        ...
        private void bandedGridView1_EndSorting(object sender, EventArgs e)
        {
            ...
            if (sender is GridView)
            {
                ...
                ((AvrView)grView.Tag).GetAllColumns().ToList().FindAll(x => x.IsAggregate).ForEach(c => m_AggrCache.ClearColumn(c.UniquePath));
        ...
        private void bandedGridView1_ColumnFilterChanged(object sender, EventArgs e)
        {
            ...
            // - forget all aggregate functions cashe of view
            ((AvrView)grid.MainView.Tag).Columns.FindAll(x => x.IsAggregate).ForEach(c => m_AggrCache.ClearColumn(c.UniquePath));
        }
        ...
        {
            if (BaseSettings.ShowAvrAsterisk && e.Value == DBNull.Value)
            {
                e.DisplayText = "*";
            }
        }
        ...
        private void ChangeColumnsOrderOfBand(GridColumn grCol, bool bChangedVisibility)
        {
            ...
            {
                ((AvrViewColumn)grCol.Tag).Owner.Columns.FindAll(x => x.IsAggregate).ForEach(c => m_AggrCache.ClearColumn(c.UniquePath));
