...
namespace eidss.avr.ViewForm
{
    public sealed class ViewDetailPresenter : RelatedObjectPresenter
    {
        ...
        public void AdjustToPivotView(AvrView pivotView)
        {
            ...
            m_DbService.GetGeneralChartMapSettings();
        }
            ...
            (string rowColumn, List<AvrViewColumn> columns, Dictionary<int, DataRow> rows, AggregateCashe aggrCache, GridView view)
        {
            ...
            if (rows != null && rows.Count > 0)
            {
                ...
                foreach (int i in rows.Keys)
                {
                    ...
                    FillMapOrChartValues(columns, aggrCache, view, rows, i, nRow);
        ...
        public DataSet CreateDataForMap
            (string rowColumn, List<AvrViewColumn> columns, Dictionary<int, DataRow> rows, AggregateCashe aggrCache, GridView view, bool useDefaults)
        {
            var ds = AvrViewHelper.PrepareMapTable(View.LayoutName, columns, useDefaults);
        ...
        private static void FillMapOrChartValues
            (IEnumerable<AvrViewColumn> columns, AggregateCashe aggrCache, GridView view, Dictionary<int, DataRow> rows, int i, DataRow nRow)
        {
            ...
            (GridView view, CollectionBase colCollection, int rowIndex, DataRow row, AvrViewColumn col, ref Dictionary<int, object> dict)
        {
            switch (col.AggregateFunction)
            {
                case (long) ViewAggregateFunction.CumulativePercent:
                    return FillGridAggrFCumProcent(view, rowIndex, col.Precision, col.SourceViewColumn, ref dict);
                case (long) ViewAggregateFunction.PercentOfColumn:
                    return FillGridAggrFColProcent(view, row, col.Precision, col.SourceViewColumn, ref dict);
                case (long) ViewAggregateFunction.PercentOfRow:
                    return FillGridAggrFRowProcent(view, row, col.Precision, col.SourceViewColumn, colCollection);
                case (long) ViewAggregateFunction.Proportion:
                    return FillGridAggrFProportion(view, row, col.Precision, col.SourceViewColumn, col.DenominatorViewColumn);
                case (long) ViewAggregateFunction.Ratio:
                    return FillGridAggrFRatio(view, row, col);
                case (long) ViewAggregateFunction.MaxOfRow:
                    return FillGridAggrFMinMax(view, row, col.Precision, colCollection, true);
                case (long) ViewAggregateFunction.MinOfRow:
                    return FillGridAggrFMinMax(view, row, col.Precision, colCollection, false);
            }
            return DBNull.Value;
        }

        private object FillGridAggrFCumProcent
            (GridView view, int rowIndex, int? precision, string refColumn, ref Dictionary<int, object> dict)
        {
            ...
            if (!Utils.IsEmpty(refColumn))
            {
                ...
                DataView data = new DataView(((DataView)view.DataSource).Table);
                data.Sort = ((AvrView)view.Tag).GetSortExpression();
                foreach (DataRowView row in data)
                {
                    ...
                    if ((long)row["ID"] == rowIndex + 1)
                        break;
                }
                /*
                 * summing of visible cells
        ...
        private object FillGridAggrFColProcent
            (GridView view, DataRow row, int? precision, string refColumn, ref Dictionary<int, object> dict)
        {
            object ret = DBNull.Value;
        ...
        // get Sum of Column
        private double GetColumnSum(GridView view, string refColumn, ref Dictionary<int, object> dict)
        {
            if (!dict.ContainsKey(-1))
            {
                double sumFull = 0;

                DataView data = (DataView)view.DataSource;
                ...
                {
                    object cur = view.GetRowCellValue(currentRowHandle, refColumn);
                    double dCur;
                    if (!Utils.IsEmpty(cur) && double.TryParse(cur.ToString(), out dCur))
                    {
                        sumFull += dCur;
        ...
        private object FillGridAggrFRowProcent(GridView view, DataRow row, int? precision, string refColumn, CollectionBase colCollection)
        {
            object ret = DBNull.Value;
        ...
        private object FillGridAggrFProportion(GridView view, DataRow row, int? precision, string refColumn, string denominator)
        {
            object ret = DBNull.Value;
        ...
        private object FillGridAggrFRatio(GridView view, DataRow row, AvrViewColumn col)
        {
            object ret = DBNull.Value;
        ...
        private object FillGridAggrFMinMax(GridView view, DataRow row, int? precision, CollectionBase colCollection, bool max)
        {
            object ret = DBNull.Value;
        ...
        public void FillMapAdminUnit(LookUpEdit cb)
        {
            ...
            if (visibleNotRowColumns.FindAll(col => col.UniquePath == View.MapAdminUnitViewColumn).Count > 0)
            {
                cb.EditValue = View.MapAdminUnitViewColumn;
            }
            else
            {
                cb.EditValue = "";
