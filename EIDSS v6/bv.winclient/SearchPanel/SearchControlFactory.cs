...
namespace bv.winclient.SearchPanel
{
    internal class SearchControlFactory: ISearchControlFactory
    {
        ...
        public DevExpress.XtraEditors.LookUpEdit CreateLookUpEdit(List<object> items, object currentValue, bool bSorting = true)
        {
        ...
        public DevExpress.XtraEditors.LookUpEdit CreateLookUpEdit(IObject obj, string lookupName, List<Tuple<string, string>> columns, object currentValue, bool bBinding)
        {
            var control = new LookUpEdit();
            ...
            list.ItemCheck += (sender, args) =>
                {
                    ...
                    try
                    {
                        var items = list.DataSource as IList;
                        if (items != null && items.Count > 0)
                        {
                            ...
                            {
                                list.SetItemCheckState(0, CheckState.Indeterminate);
                            }
                        }
                    }
                    finally
