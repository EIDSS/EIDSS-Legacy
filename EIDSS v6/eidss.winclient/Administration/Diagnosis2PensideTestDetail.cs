...
namespace eidss.winclient.Administration
{
    public partial class Diagnosis2PensideTestDetail : BaseGroupPanel_Diagnosis2PensideTest
    {
        ...
        {
            var matrix = dummyObjectWithLookups as Diagnosis2PensideTest;
            if (matrix == null)
                return;
            var column = Grid.GridView.Columns.ColumnByName("idfsPensideTestName");
            if (column != null)
                LookupBinder.BindBaseRepositoryLookup(column.SetLookupEditor(), matrix.PensideTestTypeLookup, true);
