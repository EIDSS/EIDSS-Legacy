...
namespace eidss.avr.mweb.Utils
{
    public class LayoutHelper
    {
        ...
        public void LoadPivotFromDB(AvrPivotGridData model, IList<IAvrPivotGridField> avrFields, bool isNewObject)
        {
            ...
            if (!isNewObject)
            {
                AvrPivotGridHelper.LoadSearchFieldsVersionSixFromDB(avrFields, LayoutSearchFieldTable,
            ...
            if (LayoutRow.blnShowMissedValuesInPivotGrid)
            {
                ...
                AvrPivotGridHelper.AddMissedValuesInRowColumnArea(model, avrFields);
            }
        ...
        public void PrepareWebFields(AvrPivotSettings pivotSettings)
        {
            ...
            foreach (var webField in pivotSettings.Fields)
            {
...
//        }
        ...
        private static SearchFieldType GetSearchFieldType(WebPivotGridField webField)
        {
            ...
            if (pos > 0)
            {
                string[] fieldParts = webField.GetOriginalName().Split(new[] { "__" }, StringSplitOptions.RemoveEmptyEntries);
                int i = SearchFieldLookup.Find(fieldParts[0]);
                DataRow paramRow = LookupCache.GetLookupRow(webField.GetOriginalName(), LookupTables.ParameterForFFType.ToString());
            ...
            {
                int i = SearchFieldLookup.Find(webField.GetOriginalName());
                fieldRow = SearchFieldLookup[i].Row;
        ...
        //        string[] fieldParts = webField.GetOriginalName().Split(new[] { "__" }, StringSplitOptions.RemoveEmptyEntries);
        //        int i = SearchFieldLookup.Find(fieldParts[0]);
        //        paramRow = LookupCache.GetLookupRow(webField.GetOriginalName(), LookupTables.ParameterForFFType.ToString());
        //        object referenceType = paramRow["idfsReferenceType"];
        //        isLookup = !bv.common.Core.Utils.IsEmpty(referenceType);
        //        fieldRow = SearchFieldLookup[i].Row;
        //    }
