...
{
	var c_pie = 0;
	var barColors = ['#ffff00', '#ff0000', '#0000ff', '#00ff00', '#ff00ff', '#ff7d00', '#7b3f00'];
	...
	if (map_chart_style_json)
	{
	    ...
	    if (map_chart_style['BarCharts'] && map_chart_style['BarCharts'].length) {
	        var i = 0;
	        while (i < map_chart_style['BarCharts'].length && i < 6) {
	...
	$(document).ready(function () {
		var ticks = ['1'];
		var lseriesDefaults = {rendererOptions: {barWidth: 25}, renderer: $.jqplot.BarRenderer, pointLabels: {show: true, edgeTolerance: -25}, shadow: false};
		...
		var vmin, vmax;
		var lColors = [];
		for (var i = 0; i < BarColumns.length; i++) {if (i < barColors.length - 1) {lColors.push(barColors[i]);}}
		...
		for (var i = 0; i < features.length; i++) // Get data to draw !!! features.length
		{
			...
			if (Id)
			{
			    ...
			    if (is0 != 0) //(features[i]['properties'][bar_column] == null)
			    {
			        ...
			        else { $.jqplot(Id, Values[i], { seriesColors: lColors, seriesDefaults: lseriesDefaults, axesDefaults: laxesDefaults, grid: lgrid, axes: { yaxis: { min: vmin, max: vmax } } }); }
			    }
			}
		}
...
function DrawPieLegend(BarColumns, barColors, BarColumnsName)
{
    ...
    {        
        ...
        $('#legend_grad').append("<div style='width:250px; font-size:10px; clear:both; padding:5px;'><table><tr><td width='1%'><div class='map_legend' style='float:left; background-color:" + color + "'></td><td></div><div style='float:left;'>&nbsp;-&nbsp;" + BarColumnsName[i] + "</div></td></tr></table></div>");
        i++;
    }
}
...
function LoadPolygon()
{
	...
	{     
		if (features[i]["geometry"] != null)
		{
			...
			if (!ValueColumn) {value_color = default_color;}
...
function DrawGradientLegend(min, max, start_color, end_color, steps)
{
	...
	for (var i = min; i <= max; i += step)
	{
		...
		$('#legend').append("<div style='width:55px;height:2px;background-color:" + color + ";'></div>");
	}
	...
	{     
		if (features[i]["geometry"] != null)
		{
			...
			else
			{
			    ...
			    var circleOptions = { color: value_color, fillColor: value_color, fillOpacity: 0.5, radius: value_size };
