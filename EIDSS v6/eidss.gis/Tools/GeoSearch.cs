        ...
        public class SearchItem
        {
            ...
            public string Result
            {
                get
                {
                    if (Settlement.Trim() != "")
                    {
                        return string.Format("{0} settlement, {1} rayon, {2} region", Settlement, Rayon, Region);
                    }
                   
                    if (Rayon.Trim() != "" & Settlement.Trim() == "")
                        return string.Format("{0} rayon, {1} region", Rayon, Region);

                    return string.Format("{0} region", Region);
                }
            }
                ...
                var sql =
                    string.Format(
                        @"WITH [RegSearchOrder] AS (SELECT ROW_NUMBER() OVER (ORDER BY [strTextString]) AS [rownum],
                          ...
                          WHERE [strTextString] LIKE '%{0}%') SELECT [idfsGISBaseReference], [strTextString], [Shape] FROM [RegSearchOrder] WHERE [rownum] BETWEEN {1} AND {2}",
                ...
                if (objNum < GeoObjPerPage)
                {
                    ...
                    sql = string.Format(@"WITH [RnSearchOrder] AS
                                        (
	                                        ...
	                                        WHERE [gisStringNameTranslation].[strTextString] LIKE '%{0}%' 
