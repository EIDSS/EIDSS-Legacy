...
Namespace Core
    Public Class AuditManager
        ...
        Public Shared Sub ProcessFiltration(ByVal auditData As AuditObject)
            If DontUseFiltation Then
                Return
            End If
            ...
            End If
            Dim cmd As IDbCommand
            cmd = BaseDbService.CreateSPCommand("dbo.spFiltered_CheckEvent", Connection)
            BaseDbService.AddParam(cmd, "@event", auditData.LastAuditEventID)
            SyncLock m_AsyncConnection
                Try
                    ...
                    If (Connection.State And ConnectionState.Open) = 0 Then
                        Connection.Open()
                    End If
                    CType(cmd, SqlClient.SqlCommand).BeginExecuteNonQuery(AddressOf HandleCallback, New AsyncAuditData(CType(cmd, SqlCommand), auditData))
                    Return
