...
Namespace Core
    Public Class LookupCache
        ...
        Private Shared Sub InitLookupTables()
            ...
            End If
        End Sub
        Private Shared Function GetLookupTableInfo(ByVal tableID As String, Optional ByVal HACode As Integer = 0, Optional ByVal AddAbsentTable As Boolean = True) As LookupTableInfo
            SyncLock m_LookupTables
                If m_LookupTables.ContainsKey(tableID) Then
                    Return m_LookupTables(tableID)
                ElseIf AddAbsentTable Then
                    Dim lookupTable As New LookupTableInfo(tableID)
                    If HACode > 0 Then
                        lookupTable.HACode = CInt(HACode)
                    End If
                    m_LookupTables.Add(tableID, lookupTable)
                    Return lookupTable
                End If
            End SyncLock
            Return Nothing
        ...
        Public Shared Sub CreateLookupChangedEvent(ByVal SqlTableName As String, Optional ByVal transaction As IDbTransaction = Nothing, Optional ByVal ClientID As String = Nothing)
            ...
            End If
            BaseDbService.AddParam(cmd, "@idfsEventTypeID", 10025002) '"evtLookupTableChanged"
            ...
            Trace.WriteLine(Trace.Kind.Info, "Lookup table changed (clientID {0}): {1}", ClientID, SqlTableName)
        End Sub
