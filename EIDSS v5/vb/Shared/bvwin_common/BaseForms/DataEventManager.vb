...
Public Class DataFieldChangeEventArgs : Inherits EventArgs
    Dim m_row As System.Data.DataRow
    Dim m_col As System.Data.DataColumn
    Dim m_val As Object
    Dim m_oldVal As Object
    Public Sub New(ByVal aRow As System.Data.DataRow, ByVal aColumn As System.Data.DataColumn, ByVal value As Object, ByVal oldValue As Object)
        Me.m_row = aRow
        Me.m_col = aColumn
        Me.m_val = value
        Me.m_oldVal = oldValue
    End Sub
    Public ReadOnly Property Row() As System.Data.DataRow
        ...
        End Get
    End Property
    Public ReadOnly Property Column() As System.Data.DataColumn
        Get
            Return m_col
        End Get
    End Property
...
#Region "Implementation Details"
    ...
    Private Class DataEvent
        ...
        ' call event handler
        Public Sub exec()
            m_handler(m_event.Column.Table, m_event)
        End Sub
        ...
        Private m_event As DataFieldChangeEventArgs
        Private m_handler As DataFieldChangeHandler
    End Class
    ...
    Private Sub DataChanging(ByVal Sender As Object, ByVal e As DataColumnChangeEventArgs)
        ...
        If Not m_owner Is Nothing Then
            If m_ignoreEvents = 0 AndAlso Not m_owner.Loading AndAlso Not e.ProposedValue Is Nothing AndAlso Not AreEquals(e.Row(e.Column), e.ProposedValue) Then
    ...
    Private Sub DataChange(ByVal Sender As Object, ByVal e As DataColumnChangeEventArgs)
        ' ignore changes if events are suspended, owner has no ViewNode associated or it's ViewNode is updating
        If Not m_owner Is Nothing Then
            If m_ignoreEvents = 0 AndAlso Not m_owner.Loading AndAlso m_ChangingQueue.ContainsKey(e.Column) AndAlso m_ChangingQueue(e.Column).NewValue.Equals(e.ProposedValue) Then  'Not Utils.IsEmpty(m_ChangingQueue(e.Column)) AndAlso 
                ...
                Dim handler As DataFieldChangeHandler = CType(m_eventDict(e.Column), DataFieldChangeHandler)
                If Not handler Is Nothing Then
                    ' enque the new event or replace an old one
                    Dim de As New DataEvent(New DataFieldChangeEventArgs(e.Row, e.Column, e.ProposedValue, oldValue), handler)
    ...
    Public Shared Function SubmitCurrentEdit(ByVal parent As Control) As Boolean
        ...
        Try
            ...
            If TypeOf (parent) Is DevExpress.XtraGrid.GridControl Then
                CType(parent, DevExpress.XtraGrid.GridControl).MainView.PostEditor()
                Return True
            End If

            If TypeOf (parent) Is DevExpress.XtraEditors.BaseEdit AndAlso parent.Visible Then
                For Each bind As Binding In parent.DataBindings
                    If Not bind.PropertyName Is Nothing AndAlso bind.PropertyName = "EditValue" _
                        ...
                        If Not row Is Nothing Then
                            Dim val As Object
                            Try
                                If CType(parent, DevExpress.XtraEditors.BaseEdit).EditValue Is Nothing Then
                                    val = DBNull.Value
                                Else
                                    val = CType(parent, DevExpress.XtraEditors.BaseEdit).EditValue
                                End If
                                If Not row(bind.BindingMemberInfo.BindingField).Equals(val) Then
                                    Try
                                        Dim newVal As Object = Convert.ChangeType(val, row.Table.Columns(bind.BindingMemberInfo.BindingField).DataType)
            ...
            Try 'this try block is needed to catch possible binding context collection change
                ...
                For Each de In parent.BindingContext
                    ...
                    If we.IsAlive Then
                        ...
                        If Not bmanager Is Nothing AndAlso bmanager.Count > 0 AndAlso bmanager.Position >= 0 AndAlso p_managers.ContainsKey(bmanager) = False Then
                            Try
                ...
                Next
            Catch ex As Exception
                Trace.WriteLine(Trace.Kind.Warning, "SubmitCurrentEdit fault for cotrol {0}{1}", parent.GetType().Name, parent.Name)
            End Try
...
#Region "Public API"
    ...
    Public Sub New(ByVal owner As BaseForm)
        m_owner = owner
        s_dataEventManagers.Add(New WeakReference(Me))
    ...
    Public Sub ClearAllReferences()
        ...
        For Each ref In s_dataEventManagers
        ...
        If Not toDelete Is Nothing Then
            s_dataEventManagers.Remove(toDelete)
        End If
    ...
    Public Sub RemoveDataHandler(ByVal colSpec As String)
        Dim col As DataColumn = GetColumn(colSpec)
        Dim Handler As DataFieldChangeHandler = CType(m_eventDict(col), DataFieldChangeHandler)
        If Not Handler Is Nothing Then
    ...
    Public Shared Function Flush() As Boolean
        ...
        Dim managers As ArrayList = s_dataEventManagers
        s_dataEventManagers = New ArrayList
        Dim ref As WeakReference
        For Each ref In managers
            Dim target As DataEventManager = CType(ref.Target, DataEventManager)
            If Not target Is Nothing Then
                target.DoFlush(True) ' values from all fields will be grabbed
                s_dataEventManagers.Add(ref)
            End If
        Next
    End Function
#End Region
