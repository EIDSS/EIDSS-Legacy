...
namespace EIDSS.RAM.QueryBuilder
{
    internal class FilterControlHelper : IDisposable
    {
        ...
        public FilterControlHelper(FilterControl ctl, RamPivotGrid pivotGrid, bool createEditors, CriteriaOperator filterCriteria = null)
        {
            m_FilterControl = ctl;
            if (WinUtils.IsComponentInDesignMode(m_FilterControl))
            {
        ...
        private void GenerateColumns()
        {
            ...
            var criteria = m_FilterControl.FilterCriteria;
            m_FilterControl.FilterCriteria = null;
        ...
        private void BindPivotGrid(RamPivotGrid pivotGrid, bool createEditors)
        {
        ...
        private UnboundQueryFilterColumn CreatePivotGridColumn
            (int conditionID, WinPivotGridField field, bool createEditors)
        {
            ...
            if (pos > 0)
            {
                string[] fieldParts = field.OriginalName.Split(new[] { "__" }, StringSplitOptions.RemoveEmptyEntries);
                ...
                paramRow = LookupCache.GetLookupRow(field.OriginalName, LookupTables.ParameterForFFType.ToString());
                object paramType = paramRow["idfsParameterType"];
                object referenceType = paramRow["idfsReferenceType"];
            ...
            {
                int i = m_SearchFieldsLookupView.Find(field.OriginalName);
                Dbg.Assert(i >= 0, "search field with alias {0} is not found in lookup table", fieldName);
                fieldRow = m_SearchFieldsLookupView[i].Row;
                // Commented because now caption should be taken directrly from the field caption 
...
//                    }
        ...
        public static string GetFilterTextForPivotGrid(RamPivotGrid grid)
        {
        ...
        public static string GetFilterTextForPivotGrid(RamPivotGrid grid, CriteriaOperator criteria)
        {
            using (var filter = new FilterControl())
            {
                using (var helper = new FilterControlHelper(filter, grid, false))
        ...
        public static bool CheckPivotGridPrefilter(RamPivotGrid grid)
        {
            using (var filter = new FilterControl())
            {
                using (var helper = new FilterControlHelper(filter, grid, false))
        ...
        private static WinPivotGridField GetPivotGridField(PivotGridControl grid, string fieldName)
        {
            ...
            return field as WinPivotGridField;
        }
        ...
        private void BindSpecialLookup(RepositoryItemLookUpEdit cb, string type)
        {
            ...
            switch (type)
            {
                case "Country":
                    ...
                    {
                        m_ValueHash[GisRefereneceType.Country] =LookupBinder.BindAVRGisRepositoryLookup(cb, GisRefereneceType.Country, valueMember);
                    }
                    else
                    {
                        LookupBinder.BindAVRGisRepositoryLookup(cb, GisRefereneceType.Country, valueMember, m_ValueHash[GisRefereneceType.Country]);
                    }
                    break;
                case "Region":
                    valueMember = m_DataSourceType == DataSourceType.PivotGrid ? "ExtendedName" : "idfsReference";
                    ...
                    {
                        m_ValueHash[GisRefereneceType.Region] =LookupBinder.BindAVRGisRepositoryLookup(cb, GisRefereneceType.Region, valueMember);
                    }
                    else
                    {
                        LookupBinder.BindAVRGisRepositoryLookup(cb, GisRefereneceType.Region, valueMember, m_ValueHash[GisRefereneceType.Region]);
                    }
                ...
                case "Rayon":
                    ...
                    {
                        m_ValueHash[GisRefereneceType.Rayon] =LookupBinder.BindAVRGisRepositoryLookup(cb, GisRefereneceType.Rayon, valueMember);
                    }
                    else
                    {
                        LookupBinder.BindAVRGisRepositoryLookup(cb, GisRefereneceType.Rayon, valueMember, m_ValueHash[GisRefereneceType.Rayon]);
                    }
                    break;
                case "Settlement":
                    valueMember = m_DataSourceType == DataSourceType.PivotGrid ? "ExtendedName" : "idfsReference";
                    ...
                    {
                        m_ValueHash[GisRefereneceType.Settlement] =LookupBinder.BindAVRGisRepositoryLookup(cb, GisRefereneceType.Settlement, valueMember);
                    }
                    else
                    {
                        LookupBinder.BindAVRGisRepositoryLookup(cb, GisRefereneceType.Settlement, valueMember, m_ValueHash[GisRefereneceType.Settlement]);
                    }
                    break;
                case "Organization":
                ...
                case "Site":
                    ...
                    LookupBinder.BindSiteRepositoryLookup(cb, true, false);
                    cb.ValueMember = valueMember; // "idfsOffice_Name";
                    break;
                case "AnimalAgeList":
        ...
        private bool IsNodeValid(CriteriaOperator node)
        {
            ...
            if (m_DataSourceType == DataSourceType.PivotGrid)
            {
                ...
                var field = GetPivotGridField(m_PivotGrid, fieldName);
                return (field != null) &&
        ...
        private bool ValidateNode(Node node, bool showErrorMessage)
        {
                ...
                foreach (CriteriaOperator op in ((ClauseNode)node).AdditionalOperands)
                {
                    if (ValidateOperator(op) == false)
                    {
                        if (showErrorMessage)
                        {
                            ...
                            ErrorForm.ShowWarning("ErrFilterValidatioError", "Filter criteria value for [{0}] field can't be empty", fieldCaption);
            ...
            if (groupNode != null)
            {
                var nodes = (groupNode).SubNodes.Cast<Node>();
        ...
        private static bool ValidateOperator(CriteriaOperator op)
        {
            if (op is GroupOperator)
            {
                var operands = ((GroupOperator)op).Operands;
        ...
        private static void InitSearchFieldsLookupView()
        {
            ...
            {
                row["strSearchFieldAlias"] = row["strSearchFieldAlias"] + RamPivotGridHelper.CopySuffix;
                table.ImportRow(row);
            }
            table.EndLoadData();
        ...
        private void FilterChanged(object sender, FilterChangedEventArgs e)
        {
            ...
            if (e.Action == FilterChangedAction.AddConditionNode)
            {
            ...
            if (shouldRefresh)
            {
                ((FilterControl)sender).RefreshNodes();
            }
            var handler = OnFilterChanged;
            if (handler != null)
            {
