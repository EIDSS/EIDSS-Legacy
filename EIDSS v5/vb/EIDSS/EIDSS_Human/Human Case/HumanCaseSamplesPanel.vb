...
Public Class HumanCaseSamplesPanel
    ...
    Private Sub InitializeComponent()
        ...
        Me.cbAccessionDate.VistaTimeProperties.Buttons.AddRange(New DevExpress.XtraEditors.Controls.EditorButton() {New DevExpress.XtraEditors.Controls.EditorButton(), New DevExpress.XtraEditors.Controls.EditorButton(), New DevExpress.XtraEditors.Controls.EditorButton(), New DevExpress.XtraEditors.Controls.EditorButton()})
        '
        'grpSendToOffice
        '
        ...
        'dtCollectionDate
        '
        Me.dtCollectionDate.VistaTimeProperties.Buttons.AddRange(New DevExpress.XtraEditors.Controls.EditorButton() {New DevExpress.XtraEditors.Controls.EditorButton(), New DevExpress.XtraEditors.Controls.EditorButton(), New DevExpress.XtraEditors.Controls.EditorButton()})
        '
        'colSentDate
        '
        ...
        Me.colTestDate.FieldName = "idfTesting"
        ...
        Me.colTestName.FieldName = "idfTesting"
        ...
        Me.colTestResult.FieldName = "idfTesting"
    ...
    Protected Overrides Sub DefineBinding()
        ...
        If IsSearchMode Then
            ...
            Core.LookupBinder.BindBaseRepositoryLookup(cbTestType, db.BaseReferenceType.rftTestType, HACode.All, False)
            Core.LookupBinder.BindTestResultRepositoryLookup(cbTestResult, False)
            colTestName.FieldName = "idfsTestType"
        ...
        Else
            ...
            If (baseDataSet.Tables(CaseSamples_Db.TableSamples).Select(String.Format("idfsSpecimenType = {0}", CLng(SampleTypeEnum.Unknown))).Length = 0) Then
                ...
                row("idfsSpecimenType") = CLng(SampleTypeEnum.Unknown)
                row("strSpecimenName") = LookupCache.GetLookupValue(CLng(SampleTypeEnum.Unknown), BaseReferenceType.rftSpecimenType, "name")
        ...
        End Try
        SamplesGridView.ActiveFilter.NonColumnFilter = String.Format("idfsSpecimenType<>{0}", CLng(SampleTypeEnum.Unknown))
    ...
    Protected Overrides Function ShowEditor(ByVal row As DataRow) As Boolean
        If Not row Is Nothing AndAlso (row("idfsSpecimenType").Equals(CLng(SampleTypeEnum.Unknown))) Then
            Return False
        End If
    ...
    Public Overrides Function IsCurrentSpecimenRowValid(Optional ByVal index As Integer = -1, Optional ByVal showError As Boolean = True) As Boolean
        Dim res As Boolean = MyBase.IsCurrentSpecimenRowValid(index, showError)
        If res = False Then Return False

        If index = -1 Then index = SamplesGridView.FocusedRowHandle
        If Not ValidateDate Is Nothing Then
            If Not ValidateDate(dtCollectionDate, False, showError) Then
                Return False
            End If
            If Not ValidateDate(dtSentDate, False, showError) Then
                Return False
            End If
        End If
        ...
        If True = MinMax_Err(row("datFieldCollectionDate"), row("datFieldSentDate"), "Collection Date", "Sent Date", True, showError) Then Return False
        Return True
    End Function
    ...
    Private Sub SamplesGridView_CustomRowCellEditForEditing(ByVal sender As System.Object, ByVal e As DevExpress.XtraGrid.Views.Grid.CustomRowCellEditEventArgs)
        ...
        If (e.Column Is Me.colTestName) And Not IsSearchMode Then
            ...
            Else
                TestView.RowFilter = "idfTesting = -1 OR ISNULL(idfRootParentMaterial, idfMaterial)=" + Utils.Str(row("idfRootParentMaterial"), Utils.Str(row("idfMaterial")))
            End If
        ...
        If (e.Column Is Me.colTestResult) And IsSearchMode Then
            Dim row As DataRow = Me.SamplesGridView.GetDataRow(e.RowHandle)
            TestResultView.RowFilter = String.Format("idfsTestType={0} and intRowStatus = 0", Utils.Str(row("idfsTestType"), "-1"))
            e.RepositoryItem = Me.cbTestResultDropdown
            'CType(Me.cbTestResult.DataSource, DataView).RowFilter = "idfsTestType=" + Utils.Str(row("idfsTestType"), "-1")
        End If
    End Sub
    ...
    Public Sub MergeTestTable(source As DataTable)
        ...
        For Each sRow As DataRow In source.Rows()
            sRow.EndEdit()
            If (sRow("idfContainer") Is DBNull.Value) Then
                Continue For
            End If
            If sRow("idfsTestStatus").Equals(CLng(TestStatus.Completed)) OrElse sRow("idfsTestStatus").Equals(CLng(TestStatus.Amended)) Then
                Dim tRow As DataRow = target.Rows.Find(sRow("idfTesting"))
                If tRow Is Nothing Then
                    tRow = target.NewRow()
                    tRow("idfTesting") = sRow("idfTesting")
                    tRow("idfMaterial") = sRow("idfContainer")
                    target.Rows.Add(tRow)
                End If
                If Not tRow("idfsTestType").Equals("idfsTestType") Then
                    tRow("idfsTestType") = sRow("idfsTestType")
                    tRow("TestName") = LookupCache.GetLookupValue(sRow("idfsTestType"), BaseReferenceType.rftTestType, "name")
                End If
                If Not tRow("idfsTestResult").Equals("idfsTestResult") Then
                ...
                End If
                If Not tRow("idfMaterial").Equals("idfContainer") Then
                    tRow("idfMaterial") = sRow("idfContainer")
                End If
                If Not tRow("datPerformedDate").Equals("datConcludedDate") Then
