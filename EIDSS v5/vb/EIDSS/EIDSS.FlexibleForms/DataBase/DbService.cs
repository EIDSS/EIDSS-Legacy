...
namespace EIDSS.FlexibleForms.DataBase
{
    ...
    public class DbService : BaseDbService
    {
        ...
        public bool IsParameterUsed(long idfsParameter)
        {
            ...
            //TODO нужна ли вообще проверка наличия на сервере
            /*
            if (!result)
            {
                //using (DbManagerProxy manager = DbManagerFactory.Factory.Create())
        ...
        public override bool PostDetail(DataSet ds, int postType, IDbTransaction transaction)
        {
            ...
            //проверим правильность заполнения данных

            //#region Проверка детерминантов

            ////если у шаблона есть детерминанты, то проверяем их.
            ////если у шаблона нет детерминантов, то это архивный шаблон

            ////детерминантов не может быть больше трёх на шаблон
            ...
            //#region У каждого UNI-шаблона должен быть 1 и только 1 детерминант-страна
            ...
            //        return false;
            //    }
            //    //проверим, чтоб его тип был страной
            ...
            //#region У каждого не-UNI-шаблона должно быть минимум 2 детерминанта страна+диагноз или страна+тест
            ...
            //        return false;
            //    }
            //    //проверим, чтобы была как минимум одна страна и либо диагноз, либо тест
            ...
            ////TODO может быть есть способ элегантнее и лучше
            ////среди детерминантов должны быть уникальные связки
            ////Country + Disease (Test) + FormType (детерминанты берутся по каждому шаблону)
            //for (int i = 0; i < MainDataSet.Templates.Count; i++)
            //{
            ...
            //        //проверим, не совпадают ли наборы их детерминантов
            ...
            ////отменено, потому что в таком случае невозможно создать наследника шаблона
            //#region Нельзя менять детерминанты у шаблона, у которого есть observation
            /////*
