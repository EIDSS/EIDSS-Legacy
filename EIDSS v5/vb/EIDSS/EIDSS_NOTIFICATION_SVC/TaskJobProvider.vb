...
Public Class TaskProvider
    ...
    Public Function Run(ByVal jobName As String, rc As ReplicationController) As Integer Implements IJobProvider.Run
        Try
            ...
            m_LastError = ""
            m_Task = TaskWrapper.Find(jobName)
            m_JobName = jobName
            If (m_Task Is Nothing) Then
                Dbg.Debug("Task for job {0} is not found.", jobName)
            End If

        Catch ex As Exception
            Dbg.Debug("error in Task.Create for job {0}: {1}", jobName, ex.ToString)
            If Not LastError Is Nothing Then
                m_LastError = ex.ToString
            End If
            Return 1
        ...
        Try
            m_Task.Run()
        Catch ex As Exception
            Dbg.Debug("error in Task.Run for job {0}: {1}", jobName, ex.ToString)
            If Not LastError Is Nothing Then
                m_LastError = ex.ToString
            End If
            Return 2
