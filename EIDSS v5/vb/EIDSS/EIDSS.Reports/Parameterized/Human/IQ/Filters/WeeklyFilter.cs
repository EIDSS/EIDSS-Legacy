...
namespace EIDSS.Reports.Parameterized.Human.IQ.Filters
{
    public partial class WeeklyFilter : BaseLookupFilter
    {
        ...
        public void SetCurrentDate()
        {
            ...
            if (foundrow == null)
            {
                throw new RamException("Week filter failed. Cannot select current date from the filter");
            }
        ...
        private void FillTableData(DataTable dataTable)
        {
            ...
            while (startDate.Year < EndYear)
            {
                ...
                row[KeyColumnName] = startDate;
                DateTime endDate = startDate.AddDays(6);
                row[WeekColumnNumber] = calendar.GetWeekOfYear(endDate, CalendarWeekRule.FirstFourDayWeek, DayOfWeek.Monday);
                row[StartYearColumnNumber] = startDate.Year;
                row[StartYearColumnNumber] = endDate.Year;
                string startDateFormat;
                string endDateFormat;
                if (Utils.IsEmpty(ExactFormat))
                {
                    startDateFormat = ReportRebinder.ToDateStringCurrentCulture(startDate);
                    endDateFormat = ReportRebinder.ToDateStringCurrentCulture(endDate);
                }
                else
                {
                    startDateFormat = startDate.ToString(ExactFormat);
