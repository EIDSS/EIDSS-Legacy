...
namespace bv.winclient.BasePanel
{
    ...
    public class BasePanel<T> : UserControl, IBasePanel, IApplicationForm
        where T : class, IObject
    {
        ...
        public IObject BusinessObject
        {
            ...
            set
            {
                ...
                if (m_BusinessObject != null)
                {
                    ...
                    {
                        if ((m_BusinessObject.IsNew && !Permissions.CanInsert) || !Permissions.CanUpdate)
                            m_ReadOnly = true;
                    }
                    ...
                    {
                        VisitControls(this, null, new VisitControlFunction[] { SetControlFont, CheckMandatoryField });
                    }
        ...
        private bool m_ReadonlyStateUpdated = false;
        public virtual void DisplayReadOnly()
        {
        ...
        public static string GetHelpPath(ref string language)
        {
            string path = WinClientContext.HelpFiles.ContainsKey(language) ? WinClientContext.HelpFiles[language] : "";
            if (path != "" && File.Exists(path))
            {
                return path;
            }
            if (WinClientContext.HelpFiles.ContainsKey(Localizer.lngEn))
            {
                path = WinClientContext.HelpFiles[Localizer.lngEn];
            }
            if (path != "" && File.Exists(path))
            {
                language = Localizer.lngEn;
                return path;
            }
            if (Utils.Str(BaseHelpFileName) != "")
            {
                ...
                path = BaseHelpFileName + Localizer.lngEn;
                if (File.Exists(path))
                {
                    language = string.Format("{0}.HxS", Localizer.lngEn);
                    return path;
                }
            }
            if (path != "")
        ...
        protected void ShowHelp(string topicID)
        {
            ...
            if (GetHelpPath(ref language) == null)
            {
                return;
            }
        ...
        protected virtual void ValidationEnd(object sender, ValidationEventArgs args)
        {
        ...
        protected virtual void ShowValidationError(object sender, ValidationEventArgs args)
        {
            object ctl = FindControlByPropertyName(this, null, args);
            if (ctl != null)
            {
            ...
            if (args.Type == typeof(DuplicateValueValidator))
            {
        ...
        {
            if (!(ctl is IBasePanel) || (ctl as IBasePanel).BusinessObject == null)
            {
                return;
            }
            if (boName == null)
            ...
            else
            {
                ...
                boName += (ctl as IBasePanel).BusinessObject.ObjectName;
            }
        }
        ...
        protected string GetBusinessObjectPath()
        {
            ...
            string boName = BusinessObject.ObjectName;
        ...
        protected object FindControlByPropertyName(Control ctl, string boName, ValidationEventArgs ve)
        {
            ...
            string fieldPath = ve.PropertyName;
        ...
        public void VisitControls(Control ctl, string boName, VisitControlFunction[] functions)
        {
            ...
            {
                VisitControls(c, boName, functions);
            }
        }
        ...
        {
            VisitControls(this, null, new VisitControlFunction[] { RemoveControlBindings });
        }
        ...
        protected void SetActionFunction(List<ActionMetaItem> actions, string actionName, Func<DbManagerProxy, IObject, List<object>, ActResult> actionFunction, List<object> parameters = null)
        {
