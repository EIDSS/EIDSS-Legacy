...
{
    public partial class SampleLogBookListPanel : BaseListPanel_LabSampleLogBookListItem
    {
        public SampleLogBookListPanel()
        {
            InitializeComponent();
        ...
        {
            if (notNeedCallParent != FormClosingMode.NoSave)
            {
        ...
        private class comparerLabSampleLogBookListItem : IEqualityComparer<IObject>
        {
            public bool Equals(IObject x, IObject y)
            {
                LabSampleLogBookListItem xx = x as LabSampleLogBookListItem;
                LabSampleLogBookListItem yy = y as LabSampleLogBookListItem;
        ...
        private static ActResult SelectTest(DbManagerProxy manager, IObject bo, List<object> parameters)
        {
            ...
            var s = parameters[2] as SampleLogBookListPanel;
            var focusedItem = s.FocusedItem as LabSampleLogBookListItem;
        ...
        {
            var s = parameters[2] as SampleLogBookListPanel;
            var selectedItem = s.FocusedItem as LabSampleLogBookListItem;
            if (selectedItem.idfTesting.HasValue)
            {
                var action = LabSampleLogBookListItem.Meta.Actions.Where(c => c.Name == "DeleteTest").SingleOrDefault();
        ...
        private static ActResult AddToPreferences(DbManagerProxy manager, IObject bo, List<object> parameters)
        {
            if (ErrorForm.ShowConfirmationDialog(null, EidssMessages.Get("msgAddToPreferencesPrompt", "Selected records will be added to preferences."), null) == DialogResult.Yes)
            {
                ...
                if (manager == null)
                {
                    ...
                    {
                        selectedItems.ForEach(c => LabSampleLogBookListItem.Accessor.Instance(null).AddToPreferences(manager, c as LabSampleLogBookListItem, (c as LabSampleLogBookListItem).ID));
                    }
                }
                else
                {
        ...
        /*
        private static bool RemoveFromPreferences(DbManagerProxy manager, IObject bo, List<object> parameters)
        {
            if (ErrorForm.ShowConfirmationDialog(null, EidssMessages.Get("msgRemoveFromPreferencesPrompt", "Selected records will be removed from preferences."), null) == DialogResult.Yes)
            {
                ...
                if (manager == null)
                {
                    using (manager = DbManagerFactory.Factory.Create(ModelUserContext.Instance))
                    {
                        selectedItems.ForEach(c => LabSampleLogBookListItem.Accessor.Instance(null).RemoveFromPreferences(manager, c as LabSampleLogBookListItem, (c as LabSampleLogBookListItem).ID));
                    }
                }
                else
                {
        ...
        */
        ...
        public class LabSampleLogBookListItemComparator : EqualityComparer<IObject>
        {
            public override bool Equals(IObject x, IObject y)
            {
                return (x as LabSampleLogBookListItem).idfContainer.Equals((y as LabSampleLogBookListItem).idfContainer);
            }

            public override int GetHashCode(IObject obj)
            {
                return (obj as LabSampleLogBookListItem).idfContainer.GetHashCode();
            }
        }
        ...
        private static ActResult MarkForDisposition(DbManagerProxy manager, IObject bo, List<object> parameters)
        {
            ...
            {
                var s = parameters[2] as SampleLogBookListPanel;
                var selectedItems = s.SelectedItems.Distinct(new LabSampleLogBookListItemComparator()).ToList();
        ...
        private static ActResult TransferOut(DbManagerProxy manager, IObject bo, List<object> parameters)
        {
            if (parameters[0] != null)
            {
                ...
                if (parameters.Count > 2)
                {
                    ...
                    var p = parameters[2] as SampleLogBookListPanel;
                    var selectedItems = p.SelectedItems.Distinct(new LabSampleLogBookListItemComparator()).ToList();
                    if (!CheckSampleAccessionedIn(selectedItems))
                        return false;
