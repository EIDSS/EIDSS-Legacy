...
namespace eidss.winclient.Administration
{
    public partial class VectorSubTypeDetail : BaseGroupPanel_VectorSubType
    {
        public VectorSubTypeDetail()
        {
            ...
            var acc = VectorSubType.Accessor.Instance(null);

            using (var manager = CreateDbManagerProxy())
            {
                var vectorType = acc.CreateNewT(manager, null);
                ...
                /*column = Grid.GridView.Columns.ColumnByName("strDefaultName");
                if (column != null)
                    column.SetEnglishTextEditor();*/
            }
