...
namespace eidss.winclient.Location
{
    public partial class LocationDetail : Schema.BaseDetailPanel_GeoLocation
    {
        ...
        public override void DefineBinding()
        {
            ...
            var locationTypes =
                 geoLocation.GeoLocationTypeLookup.Where(
                     c =>
                     c.idfsBaseReference != Convert.ToInt64(GeoLocationTypeEnum.Default) &&
                     c.idfsBaseReference != Convert.ToInt64(GeoLocationTypeEnum.Address)).ToList();
        ...
        protected override void ShowValidationError(object sender, bv.model.Model.Core.ValidationEventArgs args)
        {
            if (args.MessageId == "msgCoordinatesAutoCorrection")
            {
                    if (WinUtils.ConfirmMessage(EidssMessages.Get("msgCoordinatesAutoCorrection"), BvMessages.Get("Warning")))
                    {
                        geoLocation.Region =
