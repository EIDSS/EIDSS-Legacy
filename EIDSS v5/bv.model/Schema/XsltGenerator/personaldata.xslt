...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template name="personal">
    ...
    public Dictionary&lt;string, Func&lt;<xsl:value-of select="$tablename" /><xsl:value-of select="$class_suffix" />, bool&gt;&gt; m_isHiddenPersonalData;
      ...
      <xsl:for-each select="bv:item">
        ...
        <xsl:choose>
                    ...
                    <xsl:choose>
                      <xsl:when test="contains($prop2, '.')">
                        ...
                        .<xsl:value-of select="$obj3" />
                          .AddHiddenPersonalData("<xsl:value-of select="$prop3" />", c=>true);
                      </xsl:when>
                      <xsl:otherwise>
                        .AddHiddenPersonalData("<xsl:value-of select="$prop2" />", c=>true);
                      </xsl:otherwise>  
                    </xsl:choose>                    
                  </xsl:when>
                  <xsl:otherwise>
                    .AddHiddenPersonalData("<xsl:value-of select="$prop1" />", c=>true);
                  </xsl:otherwise>
                </xsl:choose>
                }
              </xsl:when>
          ...
          <xsl:otherwise>            <!-- if property is simple-->
            ...
            <xsl:otherwise>
              .AddHiddenPersonalData("<xsl:value-of select="$name" />", c=>true);
            </xsl:otherwise>
          </xsl:choose>
          </xsl:otherwise>
        </xsl:choose>
