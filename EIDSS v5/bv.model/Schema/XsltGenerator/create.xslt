...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template name="create">
        ...
        <xsl:param name="hacodable" />
            ...
            private <xsl:value-of select="$tablename" /><xsl:value-of select="$class_suffix" /> _CreateNew(DbManagerProxy manager, IObject Parent, int? HACode, on_action creating, on_action created)
            {
                try
                {
                    <xsl:value-of select="$tablename" /><xsl:value-of select="$class_suffix" /> obj = <xsl:value-of select="$tablename" /><xsl:value-of select="$class_suffix" />.CreateInstance();
                    obj.m_CS = m_CS;
                    obj.m_IsNew = true;
                    obj.Parent = Parent;
