...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template name="post">
        ...
        <xsl:param name="sp_candelete" />
        <xsl:param name="sp_candelete_shouldask" />
        ...
        <xsl:if test="$sp_post!='' or $sp_insert!='' or $sp_update!='' or (contains($sp_type,'fnlist') and $sp_delete!='')">
            ...
            public bool Post(DbManagerProxy manager, IObject obj, bool bChildObject = false)
            {
                ...
                try
                {
                    ...
                    if (!bo.GetPermissions().CanExecute("<xsl:value-of select="bv:properties/@permissionAction" />"))
                    ...
                    if (!manager.IsTransactionStarted)
                    {
                        ...
                        manager.SetEventParams("ReferenceTableChanged", new object[] { eventType, null, "<xsl:value-of select="bv:properties/@lookupcachename" />" });
                        </xsl:if>
                        if (!bo.IsNew &amp;&amp; bo.IsMarkedToDelete) // delete
                        {
                            <xsl:if test="bv:properties/@auditObject">
                        ...
                        else if (bo.IsNew &amp;&amp; !bo.IsMarkedToDelete) // insert
                        {
                            ...
                            <xsl:if test="bv:properties/@newEvent">
                            eidss.model.Enums.EventType eventType = eidss.model.Enums.EventType.<xsl:value-of select="bv:properties/@newEvent" />;
                            manager.SetEventParams("<xsl:value-of select="bv:properties/@newEvent" />", new object[] { eventType, mainObject, "" });
                            </xsl:if>
                            <xsl:if test="bv:properties/@newTestAmendmentEvent">
                        ...
                        else if (!bo.IsMarkedToDelete) // update
                        {
                            ...
                            <xsl:if test="bv:properties/@changeCaseDiseaseEvent">
                            <xsl:variable name="fields" select="util:ExpandXml('field,field1', string(bv:properties/@changeCaseDiseaseEvent), string(bv:properties/@changeCaseDiseaseEvent))" />
                            if (<xsl:for-each select="$fields/keys/key">
                            ...
                            <xsl:if test="bv:properties/@changeCaseStatusEvent">
                            if (bo.<xsl:value-of select="bv:properties/@changeCaseStatusEvent" /> != bo.<xsl:value-of select="bv:properties/@changeCaseStatusEvent" />_Original)
                            {
                            ...
                            <xsl:if test="bv:properties/@newTestResultEvent">
            ...
            <xsl:if test="$sp_candelete!=''">
                try
                {
                    if (!obj.IsForcedToDelete)
                    {
                        ...
                        if (!result) 
                        {
                            throw new ValidationModelException("<xsl:choose><xsl:when test="$msg_candelete"><xsl:value-of select="$msg_candelete"/></xsl:when><xsl:otherwise>msgCantDelete</xsl:otherwise></xsl:choose>", "_on_delete", "_on_delete", null, null, <xsl:value-of select="$sp_candelete_shouldask"/>);
                        }
                     }
                }
            ...
            public bool Validate(DbManagerProxy manager, <xsl:value-of select="$tablename" /><xsl:value-of select="$class_suffix" /> obj, bool bPostValidation, bool bChangeValidation, bool bDeepValidation, bool bRethrowException = false)
            {
                ...
                <xsl:for-each select="bv:validators/bv:change/*[not(@shouldAsk='true')]">
                    <xsl:call-template name="generate-validator">
                        <xsl:with-param name="tablename" select="$tablename" />
                    </xsl:call-template>
                </xsl:for-each>
                            ...
                            <xsl:choose>
                                ...
                                <xsl:otherwise>
                                    ...
                                    <xsl:choose>
                                        <xsl:when test="$post_type='child'">
                        if (obj.<xsl:value-of select="$post_name" /> != null)
                            foreach (var i in obj.<xsl:value-of select="$post_name" />.Where(c => !c.IsMarkedToDelete))
