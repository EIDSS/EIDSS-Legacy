...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template name="meta">
        ...
        public static class Meta
        {
            ...
            static Meta()
            {
                ...
                SearchPanelMeta.Add(new SearchPanelMetaItem(
                    ...
                    <xsl:choose><xsl:when test="@mandatory='true'">true, </xsl:when><xsl:otherwise>false, </xsl:otherwise></xsl:choose>
        ...
        <!--
                <xsl:for-each select="bv:actions/bv:action[@type='Action' or @type='Custom' or @type='Group']">
                Actions.Add(new ActionMetaItem(
                    "<xsl:value-of select="@name" />",
                    /*from BvMessages*/"<xsl:value-of select="@caption" />",
                    "<xsl:value-of select="@icon" />",
                    /*from BvMessages*/"<xsl:value-of select="@tooltip" />",
                    /*from BvMessages*/"<xsl:value-of select="@captionReadOnly" />",
                    "<xsl:value-of select="@iconReadOnly" />",
                    "<xsl:value-of select="@tooltipReadOnly" />",
                    "<xsl:value-of select="@permissionAction" />",
                    <xsl:choose><xsl:when test="@permissionPredicate"><xsl:value-of select="@permissionPredicate" />,</xsl:when><xsl:otherwise>null,</xsl:otherwise></xsl:choose>
                    ActionsAlignment.<xsl:choose><xsl:when test="@alignment"><xsl:value-of select="@alignment" /></xsl:when><xsl:otherwise>Left</xsl:otherwise></xsl:choose>,
                    ActionsPanelType.<xsl:choose><xsl:when test="@panel"><xsl:value-of select="@panel" /></xsl:when><xsl:otherwise>Main</xsl:otherwise></xsl:choose>,
                    ActionsAppType.<xsl:choose><xsl:when test="@app"><xsl:value-of select="@app" /></xsl:when><xsl:otherwise>All</xsl:otherwise></xsl:choose>,
                    true,
                    <xsl:choose><xsl:when test="@visible"><xsl:value-of select="@visible" />,</xsl:when><xsl:otherwise>true,</xsl:otherwise></xsl:choose>
                    <xsl:choose><xsl:when test="@close"><xsl:value-of select="@close" />,</xsl:when><xsl:otherwise>false,</xsl:otherwise></xsl:choose>
                    <xsl:choose>
                        <xsl:when test="@type='Group'">
                    null,
                        </xsl:when>
                        <xsl:otherwise>
                    (manager, c, pars) => { Accessor.Instance(null).<xsl:value-of select="@name" />(manager, (<xsl:value-of select="$tablename" /><xsl:value-of select="$class_suffix" />)c, pars); return true; },
                        </xsl:otherwise>
                    </xsl:choose>
                    null,
                    <xsl:choose><xsl:when test="@enabled">(a, c) => {<xsl:value-of select="@enabled"/>;},</xsl:when><xsl:otherwise>null,</xsl:otherwise></xsl:choose>
                    ActionTypes.<xsl:value-of select="@type" />,
                    ActionTypes.<xsl:choose><xsl:when test="@typeReadOnly"><xsl:value-of select="@typeReadOnly" /></xsl:when><xsl:otherwise><xsl:value-of select="@type" /></xsl:otherwise></xsl:choose>,
                    ActionTypes.<xsl:choose><xsl:when test="@typeMethod"><xsl:value-of select="@typeMethod" /></xsl:when><xsl:otherwise>Unknown</xsl:otherwise></xsl:choose>,
                    "<xsl:value-of select="@groupcode" />",
                    "<xsl:value-of select="@related-lists" />"
                    ));
                </xsl:for-each>
-->                
                    ...
                    <xsl:variable name="action_action">
                        <xsl:choose> <!-- action -->
                            <xsl:when test="$action = 'Create'">
                                <xsl:choose>
                                    <xsl:when test="contains($sp_type,'fnlist')">(manager, c, pars) => new ActResult(true, ObjectAccessor.CreatorInterface&lt;<xsl:value-of select="$cur/bv:actions/@child" /><xsl:value-of select="$class_suffix" />&gt;().CreateNew(manager, null, pars == null ? null : (int?)pars[0]))</xsl:when>
                                    <xsl:when test="contains($sp_type,'detaillist')">(manager, c, pars) => new ActResult(true, Accessor.Instance(null).CreateWithParams(manager, null, pars))</xsl:when>
                                    <xsl:otherwise>(manager, c, pars) => new ActResult(true, Accessor.Instance(null).CreateWithParams(manager, null, pars))</xsl:otherwise>
                                </xsl:choose>
                            </xsl:when>
                            ...
                            <xsl:when test="$action = 'Delete'">
                                <xsl:choose>
                                    <xsl:when test="contains($sp_type,'fnlist')">
                                        <xsl:choose>
                                            <xsl:when test="$cur/bv:storage/bv:delete">(manager, c, pars) => 
                        {
                            if (c == null)
                            {
                ...
                Actions.Add(new ActionMetaItem(
                  ...
                  <!--
                        <xsl:choose>
                            <xsl:when test="$cur/bv:actions/bv:action[@type=$action]">
                                <xsl:for-each select="$cur/bv:actions/bv:action[@type=$action]">
                    Actions.Add(new ActionMetaItem(
                        "<xsl:value-of select="@name" />",
                        /*from BvMessages*/"<xsl:value-of select="@caption" />",
                        "<xsl:value-of select="@icon" />",
                        ...
                        <xsl:choose> --><!-- create --><!--
                            <xsl:when test="$action = 'Create'">
                                <xsl:choose>
                                    <xsl:when test="contains($sp_type,'fnlist')">(manager, pars) => ObjectAccessor.CreatorInterface&lt;<xsl:value-of select="$cur/bv:actions/@child" /><xsl:value-of select="$class_suffix" />&gt;().CreateNew(manager, null, pars == null ? null : (int?)pars[0])</xsl:when>
                                    <xsl:when test="contains($sp_type,'detaillist')">(manager, pars) => Accessor.Instance(null).<xsl:value-of select="@name" />(manager, null, pars)</xsl:when>
                                    <xsl:otherwise>(manager, pars) => Accessor.Instance(null).<xsl:value-of select="@name" />(manager, null, pars)</xsl:otherwise>
                                </xsl:choose>
                            </xsl:when>
                            <xsl:when test="$action = 'Edit'">
                                <xsl:choose>
                                    <xsl:when test="contains($sp_type,'fnlist')">(manager, pars) => ObjectAccessor.SelectDetailInterface&lt;<xsl:value-of select="$cur/bv:actions/@child" /><xsl:value-of select="$class_suffix" />&gt;().SelectDetail(manager, pars[0])</xsl:when>
                                    <xsl:when test="contains($sp_type,'detaillist')">null</xsl:when>
                                    <xsl:otherwise>null</xsl:otherwise>
                                </xsl:choose>
                            </xsl:when>
                        ...
                        <xsl:choose> --><!-- enabled --><!--
                            <xsl:when test="contains($sp_type,'fnlist') and ($action != 'Create') and  ($action != 'Close') and (@action != 'Refresh')" >(a, c) => ActionMetaItem.IsListItemActionEnabled(a, c)</xsl:when>
                            <xsl:when test="$action_panel_detaillist/keys/key[@action=$action][@panel='Group'] and ($action = 'Edit' or $action = 'Delete' or @action = 'Create')">(a, c) => ActionMetaItem.IsListItemActionEnabled(a, c)</xsl:when>
                            <xsl:otherwise>
                                <xsl:choose>
                                    <xsl:when test="@enabled">(a, c) => {<xsl:value-of select="@enabled"/>;}</xsl:when>
                                    <xsl:otherwise>null</xsl:otherwise>
                                </xsl:choose>
                            </xsl:otherwise>
                        </xsl:choose>,
                    ...
                    Actions.Add(new ActionMetaItem(
                        ...
                        "<xsl:value-of select="$action_icon_readonly/keys/key[@action=$action]/@icon" />",
                        /*from BvMessages*/"<xsl:value-of select="$action_tooltip_readonly/keys/key[@action=$action]/@tooltip" />",
                        "<xsl:value-of select="$action_permission/keys/key[@action=$action]/@permissionAction" />",
                        null,
                        ActionsAlignment.<xsl:value-of select="$action_align/keys/key[@action=$action]/@align" />,
                        ActionsPanelType.<xsl:choose>
                            <xsl:when test="contains($sp_type,'fnlist')"><xsl:value-of select="$action_panel_fnlist/keys/key[@action=$action]/@panel" /></xsl:when>
                        ...
                        false,
                        true,
                        <xsl:value-of select="$action_close/keys/key[@action=$action]/@close" />,
                        <xsl:value-of select="$action_action" />,
                        <xsl:choose> --><!-- create --><!--
                            <xsl:when test="$action = 'Create'">
                                <xsl:choose>
                                    <xsl:when test="contains($sp_type,'fnlist')">(manager, pars) => ObjectAccessor.CreatorInterface&lt;<xsl:value-of select="$cur/bv:actions/@child" /><xsl:value-of select="$class_suffix" />&gt;().CreateNew(manager, null, pars == null ? null : (int?)pars[0])</xsl:when>
                                    <xsl:when test="contains($sp_type,'detaillist')">(manager, pars) => Accessor.Instance(null).CreateWithParams(manager, null, pars)</xsl:when>
                                    <xsl:otherwise>(manager, pars) => Accessor.Instance(null).CreateWithParams(manager, null, pars)</xsl:otherwise>
                                </xsl:choose>
                            </xsl:when>
                            <xsl:when test="$action = 'Edit'">
                                <xsl:choose>
                        ...
                        <xsl:choose> --><!-- enabled --><!--
                            <xsl:when test="contains($sp_type,'fnlist') and ($action != 'Create') and  ($action != 'Close') and (@action != 'Refresh')">(a, c) => ActionMetaItem.IsListItemActionEnabled(a, c)</xsl:when>
                            <xsl:when test="contains($sp_type,'fnlist') = false and $action_panel_detaillist/keys/key[@action=$action][@panel='Group'] and ($action = 'Edit' or $action = 'Delete' or @action='Create')">(a, c) => ActionMetaItem.IsListItemActionEnabled(a, c)</xsl:when>
                            <xsl:otherwise>null</xsl:otherwise>
                        </xsl:choose>,
                        ActionTypes.<xsl:value-of select="$action" />,
                        ActionTypes.<xsl:choose><xsl:when test="$action_readonly"><xsl:value-of select="$action_readonly" /></xsl:when><xsl:otherwise><xsl:value-of select="$action" /></xsl:otherwise></xsl:choose>,
                        ActionTypes.Unknown,
                        String.Empty,
                        "<xsl:value-of select="@related-lists" />"
                        ));
