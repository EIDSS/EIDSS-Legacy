...
namespace bv.common.Configuration
{
    ...
    public class ConnectionCredentials : IComparable
    {
        ...
        {
            get { return AppConfigWriter.DefaultConfigName; } 
            set { AppConfigWriter.DefaultConfigName = value; }
        }
        ...
        public void Load(string configFile, string prefix = "SQL")
        {
            ...
            if (Utils.Str(configFile) == "")
            {
                ...
                //if (ReadFromRegistry(GetKey("Server")).ToLowerInvariant() != m_SQLServer.ToLowerInvariant())
                //{
                //    WriteToRegistry(GetKey("Server"), m_SQLServer);
                //}
                //if (ReadFromRegistry(GetKey("Database")).ToLowerInvariant() != m_SQLDatabase.ToLowerInvariant())
                //{
            ...
            else
            {
                ...
                if (BaseSettings.ConnectionSource == 1)
                {
        ...
        private void SaveToRegistry()
        {
            ...
            if (sqlUserRegistry.Length > 0)
            {
                ...
                {
                    //config.GetItem("SQLUser") = Cryptor.Encrypt(Me.m_SQLUser)
                ...
                if (sqlPasswordRegistry.Length > 0)
                {
                    ...
                    if (m_SQLPassword != null && (m_SQLUser != null) && !(m_SQLPassword.Equals(sqlPasswordDecrypt)))
                    {
                        //config.GetItem("SQLPassword") = Cryptor.Encrypt(Me.m_SQLPassword, Me.m_SQLUser)
