(function ($) {
    ...
    $t.editing.initialize = function (grid) {
        ...
        if (grid.isAjax()) {
            $element.delegate('.t-grid-edit', 'click', $t.stopAll(function (e) {
                ...
                var selection = $('#' + grid.element.id + ' .t-grid-content tr.t-state-selected');
                if (!selection || selection.length == 0 || selection[0].cells.length <= 1) return;
                var edit_button_disabled = $('#' + grid.element.id + ' a.t-grid-edit.t-state-disabled');
                if (edit_button_disabled && edit_button_disabled.length > 0) return;
                grid.editRow(selection);
            }))
            ...
            .delegate('.t-grid-delete', 'click', $t.stopAll(function (e) {
                ...
                var selection = $('#' + grid.element.id + ' .t-grid-content tr.t-state-selected');
                if (!selection || selection.length == 0 || selection[0].cells.length <= 1) return;
                var del_button_disabled = $('#' + grid.element.id + ' a.t-grid-delete.t-state-disabled');
                if (del_button_disabled && del_button_disabled.length > 0) return;
                grid.deleteRow(selection);
            }))
            ...
            .delegate('.t-grid-add', 'click', $t.stopAll(function (e) {
                ...
                var add_button_disabled = $('#' + grid.element.id + ' a.t-grid-add.t-state-disabled');
                if (add_button_disabled && add_button_disabled.length > 0) return;
                grid.addRow();
            }))
    ...
    function popup(options) {
        ...
        $(options.element).one('dataBound', remove);
            ...
            if (mode == 'InLine') {
                ...
                $(this.element).find('colgroup:first').children()
                    .each(function () {
                        ...
                        if (this.className != '')
                            html.cat('<col class="').cat(this.className).cat('" />');
                        else if (width != '0px')
        ...
        this.bind = function ($ui, modelState) {
            ...
            $.each(modelState, function (key, value) {
                if ('errors' in value && value.errors[0]) {
                    $ui.find('#' + key + '_validationMessage, [data-valmsg-for="' + key + '"]')
                       .html(value.errors[0])
                       .removeClass('field-validation-valid')
                       .addClass('field-validation-error')
                       .end()
