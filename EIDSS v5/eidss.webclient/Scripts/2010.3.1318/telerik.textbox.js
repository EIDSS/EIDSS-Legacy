(function ($) {
    ...
    $t.textbox = function (element, options) {
        ...
        input.hide()
             ...
             .bind(changePropertyEventName, function (e) {
                 var attr = 'class';
                 var classValue = input.attr(attr)
                 if (classValue != inputText.attr(attr))
                     inputText.attr(attr, classValue)
             });
    ...
    $t.textbox.prototype = {
        ...
        paste: function (e, element) {
            ...
            if (selectedText && selectedText.length > 0) val = val.replace(selectedText, text);
            else val += text;
