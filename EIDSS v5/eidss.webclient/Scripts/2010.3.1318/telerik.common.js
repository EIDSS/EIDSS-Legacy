        ...
        _html: function (data) {
            ...
            if (data) {
                for (var i = 0, length = data.length; i < length; i++) {
                    ...
                    var e = {
                        html: text == undefined ? dataItem : text && text.replace(whiteSpaceRegExp, '') ? text : '&nbsp;',
                        dataItem: dataItem
                    };

                    if (this.onItemCreate) this.onItemCreate(e);
                    if (Config && Config.AutoTestingVersion) {
                        var val = dataItem.Value;
                        var v = {
                            html: val == undefined ? "" : val
                        };
                        html.cat('<li class="t-item" bvid="').cat(v.html).cat('">').cat(e.html).cat('</li>');
                    }
                    else {
