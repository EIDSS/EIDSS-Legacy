...
namespace eidss.webclient.Models
{
    public class Login
    {
        ...
        public bool ChangePassword()
        {
            ...
            {
                ErrorMessage = string.Format(Translator.GetMessageString("msgPasswordTooShort"), minimumLength.ToString());
                return false;
            }

            if (!security.ValidatePassword(NewPassword))
            {
                ErrorMessage = SecurityMessages.GetLoginErrorMessage(8);
                return false;
            }

            var change = security.ChangePassword(Organization, UserName, OldPassword, NewPassword);
            if (change == 0)
            {
                return true;
            }
            else
                ErrorMessage = SecurityMessages.GetLoginErrorMessage(change);
