...
namespace eidss.mobileclient.Utils
{
    ...
    {
        public static FilterParams GetDefaultFilter(IEnumerable<SearchPanelMetaItem> searchPanel)
        {
        ...
        public static IEnumerable<SelectListItem> GetLookup(IObjectCreator accessor, SearchPanelMetaItem item, string parameterName = null, object parameterValue = null)
        {
            ...
            string defaultValue = item.DefaultValue == null ? null : item.DefaultValue.Invoke().ToString();
                ...
                if (helpers.Length >= 3) //it means the item is interval or from combobox
                {
                    ...
                    else //interval values
                    {
                        if (helpers[2].Contains("_Day"))
                        {
                            ...
                            if (date.HasValue)
                            {
                                result.Add(helpers[2].Replace("_Day", ""), (helpers[1].ToLower() == "from") ? ">=" : "<=", date);
                            }
                        }
