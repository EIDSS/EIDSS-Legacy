...
namespace EIDSS.Reports.Parameterized.Human.AJ.Reports
{
    ...
    public partial class BaseDataQualityIndicatorsReport : BaseDateReport
    {
        ...
        public void SetParameters(DataQualityIndicatorsSurrogateModel model, DbManagerProxy manager, DbManagerProxy archiveManager)
        {
            ...
            BindHeaderLabel(periodTitle);
            BindFirstChartTitle(periodTitle, diagnosisTitle);
            BindSecondChartTitle(model, periodTitle, diagnosisTitle);
        }
        ...
        private void BindHeaderLabel(string periodTitle)
        {
            HeaderPeriodLabel.Text = periodTitle;
        ...
        private static string GetPeriod(DataQualityIndicatorsSurrogateModel model)
        {
            ...
            else
            {
                period = model.Month.Value == model.MonthEnd.Value
                    ...
                    : string.Format("{0}, {1} - {2}", model.Year, FilterHelper.GetMonthName(model.Month.Value),
        ...
        private void BindFirstChartTitle(string period, string diagnosis)
        {
            if (xrChart1.Titles.Count > 0)
            {
                xrChart1.Titles[0].Text = period + diagnosis;
            }
        }

        private void BindSecondChartTitle(DataQualityIndicatorsSurrogateModel model, string period, string diagnosis)
        {
            if (xrChart2.Titles.Count > 0)
            {
                string locationTitle = AddressModel.GetLocation(model.Language,
                    string.Empty,
                ...
                xrChart2.Titles[0].Text = period + locationTitle + diagnosis;
            }
        }
        ...
        private static string GetDiagnosisTitle(DQIDataSet.spRepHumDataQualityIndicatorsDataTable indicatorsTable)
        {
            ...
            foreach (DataRow row in diagnosisTable.Rows)
            {
                ...
                {
                    diagnosisBuilder.AppendFormat(", {0}", row[0]);
                }
            }
